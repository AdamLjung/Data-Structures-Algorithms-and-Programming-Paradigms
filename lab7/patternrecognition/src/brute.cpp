...
int main(int argc, char *argv[]) {
    ...
    double inclination;
    multimap<double, Point> point_inclines;
    for(int i = 0; i< numPoints; i++){
        point_inclines.clear();
        for(int j = i; j <numPoints; j++){
            if(j != i){
                inclination = points[i].slopeTo(points[j]);
                point_inclines.insert({inclination,points[j]});
                if(point_inclines.count(inclination) >= 3){
                    points[i].lineTo(scene, points[j]);
